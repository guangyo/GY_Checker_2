...
class ViewLastFeedbackDialog(QDialog):
    def __init__(self, fb=None,eps=None,shot=None,artist=None,parent=None,):
        super(ViewLastFeedbackDialog, self).__init__(parent)
        # 设置窗口显示样式
        ...
        # 从反馈信息中填充反馈
        if self.feedback.has_key('text'):
            self.text_feedback = self.feedback['text']
        if self.feedback.has_key('pic'):
...
if __name__ == '__main__':
    ...
    demo = ViewLastFeedbackDialog(fb=test_fb,eps='012',shot='012135(v003)',artist='Billy')
    demo.show()
    sys.exit(app.exec_())