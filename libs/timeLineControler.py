...
class TimeLineControl(object):
    ...
    def find_shot(self, eps, shot_name, rail_name):
        ...
        # 当前项目的总文件夹
        clip_bin = proj.clipsBin()

        # 初始化dst_BinItem_seq
        dst_BinItem_seq = hiero.core.BinItem('None')

        # 循环总文件夹里面的sequence
        for each in clip_bin.sequences():
            if str(eps) in each.name():
                # 目标的sequence, type is BinItem
                dst_BinItem_seq = each
